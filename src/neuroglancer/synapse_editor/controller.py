...
class Controller:
    ...
    def update_segment_display(self):
        segs = model.get_segments(self.neighbors_on)
        current_state['layers']['segmentation']['segments'] = segs
        broadcast()

    def update_synapse_display(self):
        coords = model.get_coords() if self.synapses_on else []
